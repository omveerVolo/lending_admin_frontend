<script>
	...
	let mid_enabled = page.url.searchParams.get('mid_enabled') === 'true';
	...
	let step_one = $state([
		...
		},
		{
			...
			label: 'Address',
			type: 'text',
			placeholder: 'street,city,state,country XXXX',
			handleChange: (e) => {
				let value = e.target.value;
				if (!value || value.trim() === '') {
		...
		},
		{
			...
			placeholder: 'select if any parent hospital',
			type: 'select',
			get options() {
				return parentlist;
			},
	...
	async function onSubmit() {
		...
		if (Object.keys(validationResult).length > 0) {
			...
			document.getElementById('hospital_form')?.scrollIntoView({ behavior: 'smooth' });
			window.scrollTo(0, 0);
			return;
		}
		...
		if (!isLastStep) {
			...
			try {
				...
				const response = await fetch(`${PUBLIC_API_BASE_URL}/api/hospital?${query.toString()}`, {
					method: 'POST',
					body: submissionData,
					credentials: 'include'
				});
	...
	function validateform() {
		...
		currentFields.forEach((item) => {
			...
			// --- THE AVOIDANCE LIST ---
			// We exclude these from the generic "is it empty?" check
			const excludedKeys = [...lenderKeys, 'parent', 'reimbursementMid'];
			if (excludedKeys.includes(item.key)) return;

			// Skip validation if the field is hidden in the UI
		...
		// 2. Step 1 Custom Logic
		if (step === 1 && mid_enabled) {
			// --- Parent vs Lender (Only if NOT editing) ---
			if (!edit) {
				// Requirement: Either a Parent OR at least one Lender
				if (!hasParent && !anyLenderSelected) {
					validation_errors.parent = 'Select a parent hospital or choose at least one lender';
					lenderKeys.forEach((key) => {
						validation_errors[key] = 'Select at least one lender if no parent hospital';
					});
				}
			}

			// --- Lender MID & Priority Validation ---
	...
	function populationHelper(item) {
		if (item.type == 'checkbox') {
			...
			return (formData[item.key] = item.placeholder);
		}
	}
	...
	$effect(() => {
		if (edit === true) {
			...
			const loadData = async () => {
				try {
					const res = await fetch(`${PUBLIC_API_BASE_URL}/api/hospitals/${hospital_id}`, {
						credentials: 'include'
					});
...
<div
	id="hospital_form"
	class=" grid grid-rows-[auto_1fr] w-full relative mb-16 px-0 lg:px-56 bg-gray-100"
>
...
{#snippet formTitle()}
	<div class="w-full flex flex-col items-center lg:mb-10 mb-10">
		<h1 class="text-sm font-medium uppercase tracking-[0.2em] text-slate-500 text-center">
			Edit Hospital form
		</h1>
	</div>
{/snippet}
