	...
	async function onSubmit() {
		...
		if (!isLastStep) {
			...
			try {
				...
				const response = await fetch(`https://backend.finnova.health//api/hospital?${query.toString()}`, {
					method: 'POST',
					body: submissionData,
					credentials: 'include'
				});
	...
	$effect(() => {
		if (edit === true) {
			...
			const loadData = async () => {
				try {
					const res = await fetch(`https://backend.finnova.health//api/hospitals/${hospital_id}`, {
						credentials: 'include'
					});
