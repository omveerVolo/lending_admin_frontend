<script>
	...
	async function handleClosure() {
		...
		try {
			...
			const res = await fetch(`${PUBLIC_API_BASE_URL}/api/order/closure`, {
				method: 'POST',
				credentials: 'include',
				headers: {
	...
	const orderConfig = [
		...
		},
		{
			...
			onclick: (row) => loginAsHospital(row._id)
		}
	];
	...
	async function loginAsHospital(hospitalId, event) {
		...
		try {
			const res = await fetch(`${PUBLIC_API_BASE_URL}/api/sso/generate`, {
				method: 'POST',
				credentials: 'include',
			...
			const data = await res.json();
			if (res.ok && data.loginLink) {
				window.open(data.loginLink, '_blank');
			} else {
				toast.update('Error', data.message || 'failed to generate login link', 'failed');
			}
		} catch (err) {
			console.error(err);
			toast.update('Error', err.message, 'failed');
		}
	}
	...
	async function loadData() {
		...
		try {
			const response = await fetch(`${PUBLIC_API_BASE_URL}/api/get_hospitals?${params.toString()}`, {
				method: 'GET',
				credentials: 'include'
			});

			let { hospitals, totalPages: pageCount } = await response.json();
		...
		<div class=" self-start flex flex-col gap-3 w-full max-w-md">
			...
			<div class="relative group">
				...
				<input
					...
					bind:value
					type="text"
					placeholder="Search by ChildId, RohiniId,Hospital Name"
				...
				<div class="flex items-center justify-between gap-4">
					{#if column.type == 'action_button'}
						<div class="flex flex-col truncate">
							...
							>
								<span class="text-sm">Login</span>
								<LogIn size={18} class="group-hover:-rotate-45 transition-transform duration-300" />
							</button>
						</div>
					{:else}
						<div class="flex flex-col truncate">
